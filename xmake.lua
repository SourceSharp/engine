    ...
    function()
        ...
        on_config(
            function(target)
                ...
                local ConstructEnvVar = function(sdk_engine)
                    local upper_case = string.upper(sdk_engine)
                    local ret = {string.format("HL2SDK_%s", upper_case)}
                    -- By default, linux is bash, so the variable following general naming rules, aka: `-` is not allowed.
                    -- But windows allows you to do so, so, we provide this exception.
                    if is_plat("windows") then 
                        table.insert(ret, string.format("HL2SDK-%s", upper_case))
                    end
                    return ret
                end
                ...
                local SDK_TABLE = {
                    ["csgo"] = {env = ConstructEnvVar(sdk_engine), code = 21, ext = "2.csgo", path = "csgo"}
                }
